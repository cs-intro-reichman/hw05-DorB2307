...
public class GameOfLife {
	...
	static int cellValue(int[][] board, int i, int j) {
		int aliveCells = count(board, i, j);
		if (board[i][j] == 1) {
			if (aliveCells < 2) {
				return 0;
			}
    ...
    // We use a fixed-size canvas of 900 pixels by 900 spixels for displaying game boards of different sizes.
